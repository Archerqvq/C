...
// Separate the first line and the other lines in different loot
...
// Put all days in a for loot
void calendar(int year, int daycode)
{
    for (int month = 0; month < MONTHS_IN_A_YEAR; month++)
    {
        int day = 1;                     // Start with the first day of the month
        printf("\n\n%s", months[month]); // Print month name
        printf("\n  Sun  Mon  Tue  Wed  Thu  Fri  Sat\n");
        ...
        for (; day <= days_in_month[month]; day++)
        {
            ...
            // Assert whether need to go to next line
            if (((day % 7) + daycode) % 7 == 0)
            {
                printf("\n");
            }
        }
...
int main(void)
{
    ...
    determineleapyear(year);          // Determine if it's a leap year and adjust February
